        ...
        <div>
            ...
            <div>
                <div className="eventCardAE">
                    <div>
                        <div>Organizer-ID : {(item.ORGID).slice(0,10)}</div>
                        <div>Organizer Name : {(item.NAME).toUpperCase() }</div>
                        <div>Manager : {(item.MANAGER)}</div>
                        <div>Contact : {(item.CONTACT1)}</div>
                        <div>Mail : {(item.EMAIL)}</div>
                        <div>Rating : {4.6}</div>
                        <div>Budget Proposed : {(item.NEWBUDGET)}</div><br/>
                    </div>
                    <br/>
                    <button onClick={()=>acceptEvent(item.EVENTID,item.USERNAME,item.ORGID)}>Accept</button>
